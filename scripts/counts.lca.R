...
# Data
f <- list.files(path = "mrca/", pattern = "*.mrca.tsv", full.names = TRUE)
dfs <- lapply(f, function(x) read.csv2(x, header=T, sep="\t") %>% mutate(file = str_replace(str_replace(x,".mrca.tsv",""),"mrca//","")))
...
# Write table
write.table(df,file="mrca.taxa.count.tsv",quote=F,sep="\t",row.names=F)
